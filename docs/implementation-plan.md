...
#### Phase 13b — first non-TS pack: **Python** ✅ (code-complete; live Python run still to verify)
...
- **Built:** `PYTHON` pack (pytest, best-effort pip install, `.py` sources, pytest test-file + import conventions) + `toolchainById` for reloading a run's pack. The run's pack is resolved from the GitHub language at intake and **persisted as `context.toolchainId`**; `readRunContext` re-resolves it, and implement/fix/architect/clarifier all thread it into the sandbox (`openSandbox`), the test-conventions probe, the TDD context (example-test discovery via `toolchain.isTestFile` + injected `promptConventions`), and the repo map (`toolchain.projectManifest`). Code index widened to `.py` (sidecar `SOURCE_EXT` + fake index + repo-map excluded dirs); the example-import extractor now recognises Python `from … import`. `agents/test-author.md` made language-neutral (the concrete idioms come from the pack).
- **Verified in CI:** unit tests prove the Python pack, per-run persistence + reload (`context.toolchainId`), a Ruby repo still refused, and a Python-pack TDD run discovering Python example tests + injecting pytest conventions. **Not yet verified:** a real end-to-end Python issue→PR against a live repo (needs a Python-capable sandbox image — see `docs/setup.md`), same gated posture as E2B/Anthropic.
